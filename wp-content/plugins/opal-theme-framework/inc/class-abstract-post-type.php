...
/**
 * Class OTF_Custom_Post_Type_Abstract
 */
abstract class OTF_Custom_Post_Type_Abstract {
    public $link_image;
    public $options;
    ...
    public function get_icon($name) {
        ...
        if (file_exists( OPAL_THEME_FRAMEWORK_PLUGIN_DIR . '/assets/images/post-type/' . $name . '.png' )){
            return OPAL_THEME_FRAMEWORK_PLUGIN_URL . '/assets/images/post-type/' . $name . '.png';
        } else{
    ...
    protected function init_meta_box($cbm2, $args = array(), $name) {
        $name = wp_basename( $name, '.php' );
        $args = apply_filters( 'otf_metabox_' . $name . '_fields', $args );
        foreach ($args as $arg) {
            $cbm2->add_field( $arg );
        }
    }

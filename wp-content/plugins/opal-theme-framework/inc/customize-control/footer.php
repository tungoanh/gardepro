...
class OTF_Customize_Control_Footers extends WP_Customize_Control {
        ...
        <div class="opal-control-image-select opal-control-footer" data-id="<?php echo esc_attr( $this->id ) ?>">
            <select <?php $this->link(); ?> title="">
                ...
                foreach ($footers as $footer):
                    echo '<option value="' . esc_attr( $footer->ID ) . '"' . selected( $this->value(), $footer->ID, false ) . '>' . esc_html( $footer->post_title ) . '</option>';
                    ?>
                <?php endforeach; ?>
            </select>
        </div>

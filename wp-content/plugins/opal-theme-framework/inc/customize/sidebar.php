...
if (get_theme_mod( 'otf_layout_sidebar_is_boxed', false )){
    $sidebar_background_color = get_theme_mod( 'otf_colors_sidebar_bg_color', '#fff' );
    $sidebar_border_color     = get_theme_mod( 'otf_colors_sidebar_border_color', '#ddd' );
    $sidebar_padding          = get_theme_mod( 'otf_layout_sidebar_padding_inside_boxed', 30 );
    $sidebar_layout_prefix    = '';
    $border_radius            = get_theme_mod( 'otf_layout_sidebar_sidebar_border_radius', 0 );
...
$sidebar_color            = ariColor::new_color( get_theme_mod( 'otf_colors_sidebar_color', '#222' ) );
if (get_theme_mod('otf_colors_general_primary','#0160b4') == get_theme_mod( 'otf_colors_sidebar_color', '#222' )) {
    $sidebar_color_hover      = $sidebar_color->get_new( 'lightness', $sidebar_color->lightness - 15 );
}else {
    $sidebar_color_hover = ariColor::new_color( get_theme_mod( 'otf_colors_general_primary', '#0160b4' ) );
}


if(get_theme_mod( 'otf_colors_sidebar_color', '#222' ) != '#222') {
    $cssCode .= <<<CSS
    #secondary *,#secondary .widget_recent_entries ul li:before, #secondary .widget_pages ul li:before, #secondary .widget_categories ul li:before, #secondary .widget_archive ul li:before, #secondary .widget_recent_comments ul li:before, #secondary .widget_nav_menu ul li:before, #secondary .widget_links ul li:before, #secondary .widget_product_categories ul li:before, #secondary .widget_layered_nav ul li:before, #secondary .widget_layered_nav_filters ul li:before,
    #secondary .widget a:not(.button)
    {
    ...
    #secondary .widget a:hover:not(.button),
    #secondary .widget a:active:not(.button){
    color: {$sidebar_color_hover->to_css( 'rgba' )};
}
CSS;
}

$font_size      = get_theme_mod( 'otf_typography_sidebar_title_font_size', 16 );
$letter_spacing = get_theme_mod( 'otf_typography_sidebar_title_letter_spacing', 1 );
$padding_top    = get_theme_mod( 'otf_typography_sidebar_title_padding_top', 15 );
$padding_bottom = get_theme_mod( 'otf_typography_sidebar_title_padding_bottom', 15 );
$margin_top     = get_theme_mod( 'otf_typography_sidebar_title_margin_top', 20 );
$margin_bottom  = get_theme_mod( 'otf_typography_sidebar_title_margin_bottom', 20 );
$color_title    = get_theme_mod( 'otf_colors_sidebar_title_color', '#222' );
...
$font_style              = get_theme_mod( 'otf_typography_sidebar_font_style' );

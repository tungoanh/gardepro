...
class OTF_Field_Agent_Info {
    ...
    private function render_content($userid) {
        ...
        if ($phone) {
            $phone = '<strong>' . __( 'Phone', 'opal-theme-framework' ) . '</strong>: ' . $phone . '<br>';
        }
        if (!$avatar) {
        ...
        <div class="meta-agent-info">
            ...
            <div class="info">
                ...
                <address>
                    <strong><?php _e( 'Email', 'opal-theme-framework' ) ?></strong>: <?php echo $user_info->user_email; ?>
                    <br>
                    <?php echo $phone; ?>
                </address>
                <a href="<?php echo esc_url( $link_profile ) ?>"
                   class="button button-primary button-large"><?php _e( 'View Profile', 'opal-theme-framework' ) ?></a>
                <a href="<?php echo esc_url( $link_properties ) ?>"
                   class="button button-primary button-large"><?php _e( 'View all Properties', 'opal-theme-framework' ) ?></a>
            </div>
        </div>
        <?php
    }

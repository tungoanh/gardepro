...
if ( 'custom' === $style ) {
    ...
    if(!in_array($style, array(
        ...
        $button_class .= ' vc_general vc_btn3-color-' . $color . ' ';
    }else{
        $button_class .= ' ' .$style;
    }
}
...
$link_output = '';
if ( $use_link ) {
    $link_output = vc_gitem_create_link_real( $atts, $post, 'vc_btn3 ' . trim( $button_class ), $title );
}
...
elseif ( 'load-more-grid' === $link ) :
    $output .= '<a href="javascript:;" class="vc_btn3 ' . esc_attr( $button_class ) . '" ' . $inline_css . ' ' . $attributes . '>' . $button_html . '</a>';
else :
    $output .= '<button class="vc_btn3 ' . esc_attr( $button_class ) . '"' . $inline_css . ' ' . $attributes . '>' .
        $button_html . '</button>';
endif;
$output .= '</div>';
